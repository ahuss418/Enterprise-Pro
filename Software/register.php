<?php
//Humairah and Zainah both did the registration page. 
require_once('includes/setup.php');


// Logged in users should not be able to access the register page
if ($loggedInUser) {
	redirect('index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!$liveLocal && !$workLocal && !$studyLocal) {
		flash('error', 'You must select at least one local area status to register.');
		redirectToReferer();
	}
	...
	// Double check that both of the passwords work
    ...
    INSERT INTO users (
        ...
        admin, verified, email_confirmed, reset_token, reset_token_expire, auth_secret
    ) VALUES (
        '{$firstName}', '{$lastName}', '{$email}', '{$passwordHashed}', 
        '{$addressLine1}', '{$addressLine2}', '{$town}', '{$county}', '{$postcode}', 
        {$liveLocal}, {$workLocal}, {$studyLocal}, 
        {$admin}, {$verified}, {$email_confirmed}, {$liveLocal}, {$workLocal}, {$studyLocal}
    )");
...
<div class="block block--auth">
	...
	<div class="block__body">
		...
		<form action="" method="POST">
			...
			<div class="input">
				<label class="input__label">Local Area Status</label>
				<div class="checkbox">
					<input type="checkbox" id="input-live-local" name="live-local">
					<label for="input-live-local">I live in the local area</label>
				</div>
				<div class="checkbox">
					<input type="checkbox" id="input-work-local" name="work-local">
					<label for="input-work-local">I work in the local area</label>
				</div>
				<div class="checkbox">
					<input type="checkbox" id="input-study-local" name="study-local">
					<label for="input-study-local">I study in the local area</label>
				</div>
			</div>
